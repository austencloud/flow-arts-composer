...
- Reviews, explanations, investigations, and recommendations are read-only. A
  direct request to build, fix, update, rewrite, or implement authorizes that
...
- Do not expand approved work into adjacent cleanup, deployment, external-data
  mutation, purchases, messages, or personal-account actions. Preserve
  unrelated and in-flight changes.
...
- Completed implementation must be on local `main` or in a verified, live
  worktree preview; a branch name or path alone is not delivery. Approval
  includes scoped commits, guarded local integration, and clean worktree
  removal. From the primary checkout run `npm run wt:finish -- <branch> --route
/real-route` for reviewable UI or `npm run wt:finish -- <branch> --nonvisual`
  otherwise, without a routine permission question unless Austen says not to
  merge.
...
- If guarded integration is blocked or work remains unmerged for preview, keep
  the branch and worktree. For an app route, start or reuse a task-owned server
  rooted in that worktree on a free non-5173 port after the resource gate;
  verify the route against worktree content, open it in DevTools or the in-app
  browser when available, and provide its clickable URL.
- Keep a handed-off preview server and delivered browser tab alive past the
  final response. Use a persistent or detached process if needed; record the
  task-owned process, port, worktree, and log in task context, reserve its
  Vite-server slot, and stop it when superseded by integration or when Austen
  finishes the task.
- Nonvisual instruction or code artifacts should prefer integration and need no
  fake app server. If integration remains blocked, provide a reviewable
  artifact or diff and report it accurately. If neither delivery path works,
  report the blocker and attempted alternatives without claiming completion.
  Never delete another task's branch or dirty worktree.

## Verification

- Match evidence to risk and stop after appropriate checks pass. Documentation
  and instructions need formatting, reference resolution, and focused contract
  checks, not the full Svelte check. Code needs the closest silent-regression
  test plus the narrowest relevant type, lint, or build check.
...
- New surfaces, substantial restyles, and aesthetic reviews also use
  ...
  establish aesthetic acceptance; check the dated research ledger through that
  workflow and do not claim an uncalibrated reviewer detects AI authorship.
- Localhost verification in the dedicated agent browser is approved UI work;
  external mutation and Austen's personal session still require authorization.
