...
describe("composition round trip over the generated LOOP corpus", () => {
  ...
  it("preserves the per-beat letter, which the wire format does not", () => {
    for (const { sequence } of corpus) {
      const back = hydrate(asStoredDocument(ensureComposition(sequence)));
      expect(back.steps.map((s) => s.letter)).toEqual(
        sequence.steps.map((s) => s.letter)
      );
    }
  });
...
describe("legacy hand-identity migration at ingress", () => {
  it("maps blue/red motion keys onto left/right without losing metadata", () => {
    const legacy = {
      ...
      creatorIntent: {
        propConfig: { bluePropType: "staff", redPropType: "club" },
      },
    };
    ...
    const motions = step["motions"] as unknown as Record<
      string,
    ...
    const pairing = (
      out["stepPairings"] as unknown as Record<string, never>[]
    )[0]!;
