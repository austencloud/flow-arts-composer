...
/** Every <circle fill="FILL" cx="…" cy="…" r="…"/> in an SVG string. */
function circles(
  svg: string,
  fill: string
): { cx: number; cy: number; r: number }[] {
  const re = new RegExp(
    `<circle fill="${fill}" cx="([-\\d.]+)" cy="([-\\d.]+)" r="([-\\d.]+)"`,
    "g"
  );
...
/**
 * The x coordinates of the first <path fill="FILL" ...d="…"/> in an SVG
 * string. Walks the d string command by command (M, L, A, Z) rather than
 * pulling every "number,number" pair, because an arc command's radii
 * (`A rx,ry …`) look exactly like a coordinate pair to a plain regex.
 */
function pathXs(svg: string, fill: string): number[] {
  const pathMatch = new RegExp(`<path fill="${fill}"[^>]*\\sd="([^"]+)"`).exec(
    svg
  );
  if (!pathMatch) throw new Error(`no path with fill ${fill}`);
  const tokens = pathMatch[1].match(/[MLAZ][^MLAZ]*/g) ?? [];
  const xs: number[] = [];
  for (const token of tokens) {
    const nums = token
      ...
      .filter(Boolean)
      .map(Number);
    if (token[0] === "M" || token[0] === "L") xs.push(nums[0]);
    // A takes rx, ry, x-axis-rotation, large-arc-flag, sweep-flag, x, y.
    if (token[0] === "A") xs.push(nums[5]);
  }
  return xs;
}
...
describe("generated hoop family artwork", () => {
  ...
  it("writes the pictograph, animated and button files identically", () => {
    for (const prop of ["minihoop", "bighoop", "triangle"]) {
      const a = readFileSync(
        `static/images/props/pictograph/${prop}.svg`,
        "utf8"
      );
      expect(
        readFileSync(`static/images/props/animated/${prop}.svg`, "utf8")
      ).toBe(a);
      expect(
        readFileSync(`static/images/props/buttons/${prop}.svg`, "utf8")
      ).toBe(a);
    }
  });
  ...
  it("paints the hardware where the hand and the far rim are", () => {
    // Corner grip: the gold elbow sphere sits on the grip, at the box
    // centre; both black elbow spheres are on the far side, past centre.
    const triangle = readFileSync(
      "static/images/props/pictograph/triangle.svg",
      "utf8"
    );
    const centreX = HOOP_FAMILY_BOXES.triangle.width / 2;
    const centreY = HOOP_FAMILY_BOXES.triangle.height / 2;
    const gold = circles(triangle, "#C9AC68");
    expect(gold).toHaveLength(1);
    expect(Math.abs(gold[0].cx - centreX)).toBeLessThanOrEqual(0.01);
    const black = circles(triangle, "#1C1C1F");
    expect(black).toHaveLength(2);
    for (const c of black) {
      expect(c.cx).toBeGreaterThan(centreX);
    }

    // Side grip: no vertex is gripped, so all three elbows are black. The
    // far vertex is the one sitting on the axis (cy at the box centre), and
    // it must be well past centre, not stranded near the grip.
    const side = readFileSync(
      "static/images/props/appearances/triangle-side.svg",
      "utf8"
    );
    const farVertex = circles(side, "#1C1C1F").find(
      (c) => Math.abs(c.cy - centreY) < 0.01
    );
    expect(farVertex).toBeDefined();
    expect(farVertex!.cx).toBeGreaterThan(centreX + 100);

    // Minihoop: the gold grip band sits at the near (left) rim, every x
    // behind the ring centre; the black join tape sits at the far (right)
    // rim, every x past it.
    const mini = readFileSync(
      "static/images/props/pictograph/minihoop.svg",
      "utf8"
    );
    const ringCentreX = HOOP_FAMILY_BOXES.minihoop.width / 2 + 59.4;
    for (const x of pathXs(mini, "#C9AC68")) {
      expect(x).toBeLessThan(ringCentreX);
    }
    for (const x of pathXs(mini, "#1C1C1F")) {
      expect(x).toBeGreaterThan(ringCentreX);
    }
  });
  ...
  it("crops each button to its painted window", () => {
    for (const key of ["minihoop", "bighoop", "triangle"] as const) {
      ...
      expect(crop.imageHeight).toBe(HOOP_FAMILY_BOXES[key].height);
      expect(crop.x).toBeGreaterThanOrEqual(0);
      expect(crop.x + crop.width).toBeLessThanOrEqual(crop.imageWidth + 0.01);
      expect(crop.y).toBeGreaterThanOrEqual(0);
      expect(crop.y + crop.height).toBeLessThanOrEqual(crop.imageHeight + 0.01);
    }
  });
