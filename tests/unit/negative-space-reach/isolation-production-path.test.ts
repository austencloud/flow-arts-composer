...
import { ISOLATION_SEQUENCE, ISOLATION_STAFF_LENGTH_CM } from "../../../src/routes/test/negative-space-reach/isolation-loop";
  ...
  it("visits south, east, north, west with only the right prop", () => {
    ...
    try {
      ...
      for (let phase = 0; phase < 4; phase += 0.125) {
        ...
        const radial = new Vector3(right.worldPosition.x, right.worldPosition.y, 0);
        // Staff3D maps its tracked +Y end onto -X before the score rotation.
        const thumbAxis = new Vector3(-1, 0, 0).applyQuaternion(right.worldRotation);
        expect(thumbAxis.dot(radial.clone().normalize())).toBeCloseTo(-1, 5);
        const anchored = radial.addScaledVector(thumbAxis, ISOLATION_STAFF_LENGTH_CM / 200);
        expect(anchored.length()).toBeLessThan(1e-6);
      }
      expect(state.propStatesAtScoreTime(4)).toEqual(state.propStatesAtScoreTime(0));
    } finally {
      state.destroy();
    }
  });
