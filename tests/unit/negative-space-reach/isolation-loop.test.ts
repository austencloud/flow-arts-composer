...
import { captureIsolationSnapshot, DEFAULT_TORSO_KEYFRAMES, restoreIsolationSnapshot, selectedIsolationKeyframe, torsoYawAt, torsoYawAtKeyframes, upsertTorsoKeyframe, wrapIsolationPhase } from "../../../src/routes/test/negative-space-reach/isolation-loop";
...
describe("single-hand isolation loop", () => {
  ...
  it("interpolates the torso smoothly through the cyclic seam", () => {
    expect(torsoYawAt(0, DEFAULT_TORSO_KEYFRAMES)).toBe(torsoYawAt(4, DEFAULT_TORSO_KEYFRAMES));
    expect(torsoYawAt(2.5, DEFAULT_TORSO_KEYFRAMES)).toBeLessThan(DEFAULT_TORSO_KEYFRAMES[2]!);
  });

  it("restores pose settings without sharing snapshot arrays", () => {
    const saved = captureIsolationSnapshot("data:image/png;base64,real", 4.25, [{ phase: 0.25, yaw: 0.1 }]);
  ...
  it("adds an editable keyframe at the selected moment and keeps the loop smooth", () => {
    const edited = upsertTorsoKeyframe([{ phase: 0, yaw: 0 }, { phase: 2, yaw: -0.8 }], 2.5, -1);
