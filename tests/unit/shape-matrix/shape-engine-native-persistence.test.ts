...
describe("installed Shape Engine view persistence", () => {
  it("restores a ratio playground selection after the app closes", () => {
    const state = readShapeMatrixRouteState(
      "theory=1&leftRatio=3:2&rightRatio=5:4&mode=QO"
    );
  ...
  it("shares a public web address instead of the device's localhost address", () => {
    const state = readShapeMatrixRouteState(
      "level=3&blueTurn=0.5&redTurn=1.5&mode=SS"
    );
  ...
  it("gives an incoming shared view precedence over the last saved view", () => {
    ...
    expect(persistence.restore()).toEqual(
      readShapeMatrixRouteState("level=4&mode=TO")
    );
  });

  it("saves each hand's prop into native settings when a view persists", () => {
    persistence.persist(readShapeMatrixRouteState("prop=staff&rp=fan"));
    expect(getSettings().leftPropType).toBe("staff");
    expect(getSettings().rightPropType).toBe("fan");
  });
});
