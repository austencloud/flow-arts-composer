...
function trace(steps: StepData[]): string[] {
  return steps.map(
    (s) =>
      `${s.letter ?? "-"} ${s.startPlacement}→${s.endPlacement} ` +
      `L:${s.motions.left.startLocation}→${s.motions.left.endLocation} ` +
      `R:${s.motions.right.startLocation}→${s.motions.right.endLocation}`
  );
}

function seedFor(letter: string, startPlacement: string, endPlacement: string) {
  const row = loadPictographRows("diamond").find(
    (r) =>
      r.letter === letter &&
      r.startPlacement === startPlacement &&
      r.endPlacement === endPlacement
  );
  if (!row) {
    throw new Error(
...
describe("D1 — app MIRRORED_SWAPPED_INVERTED relabels a position the hands never left", () => {
  ...
  it("app: derived step keeps the hands at w/e but calls the position alpha7", () => {
    ...
    const derived = getGridPlacementFromLocations(
    ...
    expect(app.steps[2]!.endPlacement).toBe("alpha7");

    // …and therefore the LOOP never returns home.
    expect(app.steps[2]!.endPlacement).not.toBe(seed[0]!.startPlacement);
  });
  ...
  it("root cause: the app mirrors the end position without composing the swap", () => {
    ...
    // VERTICAL_MIRROR_PLACEMENT_MAP. The composite this LOOP performs on the
    ...
    expect(app.steps[2]!.endPlacement).toBe("alpha7");
    expect(unaffected.steps[2]!.endPlacement).toBe("alpha1");
  });
});
...
describe("D2 — engine quartered ROTATED_INVERTED stops rotating on its copy pass", () => {
  ...
  it("engine: the third pass copies the seed's end locations and the orbit stalls", () => {
    ...
    expect(trace(engine.steps)).toEqual([
      ...
      // locations (n/s) and the source's endPlacement (alpha5). A pro cw
    ...
    expect(engine.steps[engine.steps.length - 1]!.endPlacement).toBe("alpha7");
    expect(seed[0]!.startPlacement).toBe("alpha3");
  });
});
