...
Status: rules approved by Austen, 2026-09-22 (see Decisions), and
implemented 2026-09-23 (see Implementation). The skewed-frame classifier and
the skewed and trigrid dataframes now follow this rule.
...
add the dash: W- X- Y- Z- Σ- Δ- Θ- Ω-. This reproduces the diamond's Type 3
rows and the skewed frame's 2026-09-21 Type 3 table, which the 2026-09-23
change left as it was. Type 3 exists only where a dash
...
| Skewed diamond frame (36)          | S T U V x2            | M N O x2; P Q R x2            | all eight             | all eight           |
...
The `--members` option prints one beat from each class with its label.
`src/lib/shared/pictograph/lettering/__tests__/multigrid-lettering.test.ts`
holds that output and requires the app to give each of those beats the label
in the Letter column, class by class, on every grid.
...
hand held at its end point. "Near/far" is filled only where the near/far
rule, which the skewed-frame classifier used until 2026-09-23, gives a
different letter (see "Why the landmark rule and not near/far").
...
Until 2026-09-23 the skewed-frame classifier (`OPPOSITE_FAMILIES` in
`src/lib/shared/pictograph/skew/skewed-frame-letter.ts`, since removed) used a
different rule. Written for any grid it reads: take the last landmark reached
after the start. Alpha gives D E F when the start spacing is under 90,
otherwise M N O. Beta gives J K L when the start spacing is over 90,
otherwise P Q R. If no landmark is reached after the start, a beta start
gives D E F and an alpha start gives J K L. Call this the near/far rule.
...
| Skewed diamond   | 45 through alpha to 135 | M N O         | D E F           |
| Skewed diamond   | 135 through beta to 45  | P Q R         | J K L           |
| Skewed pentagrid | 108 through beta to 36  | P Q R         | J K L           |

On the skewed diamond, the near/far letters are what the frame's classifier
gave until 2026-09-23.
...
- It reverses the opposite-direction table of
  `docs/superpowers/specs/2026-09-21-skewed-frame-lettering-design.md`, which
  had shipped. (Earlier versions of this document called that table the
  spec's decision 2. Its decision 2 is the brace notation, which this rule
  does not touch.) 128 of the 1024 Type 1 to 3 rows in category 3 of
  `SkewedPictographDataframe.csv` change: D to M 16, E to N 16, F to O 32,
  J to P 16, K to Q 16, L to R 32. The skewed diamond frame then uses only
  M N O and P Q R for opposite shifts.

Austen chose the landmark rule (decision 4). It was implemented on
2026-09-23, and exactly those 128 rows changed.

## Trigrid dataframe (regenerated 2026-09-23)

`static/data/pictographs/TrigridPictographDataframe.csv` (points n, se, sw)
disagreed with the catalog. On 2026-09-23 it was regenerated by
`scripts/generate-trigrid-dataframe.ts`, which letters every Type 1 and 2
beat with the shared rule and replaces the hand-written
`scripts/generate-trigrid-dataframe.cjs`. Compared with the old file:

- Size: the old file had 189 rows for 147 distinct beats. The new file has
  225 rows, one per beat: 144 Type 1, 72 Type 2 and 9 Type 6 (β with both
  hands on one point, γ with them on two).
- Duplicates: 42 beats had two rows with different letters. 36
  same-direction beats had an S T U row and a P Q R row, and 6
  opposite-direction beats had an M row and a V row.
- Missing: 78 beats had no row: F 6, I 6, J 6, K 6, L 12, R 6, U 6, V 6,
  Θ 12, Ω 12. They include every gamma to beta opposite-direction beat (the
  catalog's J K L) and every Θ Ω beat.
- Relettered: J K L from beta to gamma become D E F (18 beats). M N O from
  gamma to gamma through beta become P Q R (18 beats, including the 6 with a
  V row). 6 same-direction hybrids lettered U lead with the anti hand, so
  they become V; the old file had no same-direction V row at all.
- Rotation: every one of the old file's 150 anti hands turned with its path.
  The diamond turns every pro hand with its path and every anti hand against
  it (768 shift hands, no exceptions), and the new file does the same.
- Direction and timing follow the diamond. `direction` is same or opp when
  both hands shift and none otherwise. The diamond's same-direction rows name
  the spacing they keep in `timing` (tog 0, quarter 90, split 180), and its
  opposite-direction rows name the axis the hands mirror across (tog
  vertical, split horizontal, quarter diagonal). A triangle has only the 0
  spacing and only the vertical axis, so a trigrid row is tog when the hands
  travel together from one point or mirror each other across the vertical,
  and none otherwise.

Correction: the first version of this section said the file is read only by
`src/lib/features/lab/trigrid-lab`. That was wrong; the lab reads no CSV. The
app's CSV loader (`src/lib/shared/foundation/services/data/csv-loader.ts`)
downloads and caches the file with the other dataframes, but the motion and
letter query handlers give the trigrid grid mode no rows, and the movement
map loads only diamond and box. No screen in the app shows trigrid rows.
Tests read the file through `tests/helpers/real-pictograph-loader.ts` and
`tests/unit/combination/pictograph-dataset.ts`, and
`tests/unit/pictograph/trigrid-dataframe.test.ts` pins it to the generator.
...
Result on 2026-09-23, after the change:

- `DiamondPictographDataframe.csv`: unchanged; both rules still match all 512
  rows.
- `SkewedPictographDataframe.csv`: the landmark rule matches all 1024 rows;
  near/far differs on the same 128.
- `TrigridPictographDataframe.csv`: the landmark rule matches all 216 Type 1
  and Type 2 rows; near/far differs on 24, the P Q R class in the table
  above.
- Through the app's own letter lookup (`fuseSequences` then
  `deriveLettersForSequence`, reading the real CSVs), a two-beat Rotate 45°
  fuse, blue pro n to w then w to s and red pro ne to se then se to sw, reads
  `{DJ}` with the old skewed file and `{MP}` with the new one.
...
work, on 2026-09-22 and again on 2026-09-23. Letter facts were checked
against the dataframes and the classifier instead of the MCP.
...
3. A letter that occurs at more than one start spacing on a grid is numbered
   ...
   dash (Θ1-). For Type 3 the start spacing is read on the class path, with
   the dashing hand where its dash ends, so a Type 3 letter takes the number
   of the Type 2 letter on the same path. On the skewed pentagrid the dash
   lands on the other pentagon, so Θ1- and Θ2- follow the plain pentagrid's
   Θ1 (0 to 72) and Θ2 (72 to 144), although their hands start 180 and 108
   apart. Numbering by where the hands start would swap the two.
   The same numbering applies to the skewed diamond frame,
   where S T U V occur at 45 and 135 and, under decision 4, M N O and P Q R
   each occur from both; `skewedFrameLetterLabel` numbers them. Numbers are
   worked out from the beat by `letterLabelOnGrid`. They are not stored in
   the dataframes or added to the `Letter` enum.
4. Opposite-direction shifts use the landmark rule. This reverses the
   opposite-direction table of
   `docs/superpowers/specs/2026-09-21-skewed-frame-lettering-design.md` for
   the skewed diamond frame: the 128 category 3 rows listed above moved from
   D E F / J K L to M N O / P Q R on 2026-09-23.

## Implementation (2026-09-23)

- `src/lib/shared/pictograph/lettering/multigrid-lettering.ts` holds the rule
  once, for every grid. `LETTER_GRIDS` names the five grids (trigrid,
  diamond, pentagrid, skewedDiamond, skewedPentagrid). A beat is two hands,
  each a motion type plus start and end points counted clockwise from 0.
  `letterOnGrid` returns the letter, `letterLabelOnGrid` the letter with its
  variant number (S1, M2, Θ1-), `letterStartSpacings` the start spacings
  behind each number (for Type 3, with the dash at its end), and `gridBeats`
  every beat on a grid. A beat outside the rule, such as one with a float
  hand, gets null.
- `classifySkewedFrameLetter` in
  `src/lib/shared/pictograph/skew/skewed-frame-letter.ts` hands Types 1 to 3
  to `letterOnGrid(LETTER_GRIDS.skewedDiamond, ...)`; Types 4 to 6 are as
  before. `SKEWED_FRAME_LETTERS` lists 32 letters, down from 38.
  `skewedFrameLetterLabel` returns the numbered label.
- `scripts/generate-skewed-dataframe.ts` was rerun. In
  `static/data/pictographs/SkewedPictographDataframe.csv` exactly the 128
  rows listed above changed, and only in the letter column; the other 6144
  rows are identical. The copy in `mcp-server-pkg/assets/data/pictographs/`
  matches it. The deployed MCP server was not redeployed.
- `scripts/generate-trigrid-dataframe.ts` wrote the new trigrid file (see
  "Trigrid dataframe").
- Tests: `multigrid-lettering.test.ts` (all 512 diamond rows, every class
  of the five catalogs against the census, invariance under rotation, mirror
  and swap, time reversal, variant numbers, no letter for a float hand), `skewed-frame-letter.test.ts` (the 32 letters and their
  counts over all 1152 frame beats), `skewed-frame-dataframe.test.ts` (the
  1152 frame rows equal a fresh generator run) and `trigrid-dataframe.test.ts`
  (the whole file equals a fresh generator run).

What changes for users: a new Rotate 45° fuse letters its opposite-direction
frame beats M N O or P Q R where it used to write D E F or J K L (see the
fuse check under Evidence). No stored sequence was rewritten. Screens that
re-derive letters from the motions when they open a sequence
(`hydrateSequence` in `src/lib/shared/navigation/services/sequence-hydrator.ts`,
used by `/q/[code]`, `/sequence/[id]` and the sequence viewer drawer) will show
the new letters for those beats in older saved fuses, while a stored `word`
keeps the old letters until that sequence is saved again. Nothing in the app
displays variant numbers yet, and no pentagrid dataframe exists; both are
separate work.
