...
| Re-checked against     | Local `main` `04ae3b04` during reviewed integration. The task-owned changes applied without conflict, and the focused keyboard and Chromium component suites were rerun on that base.                                                            |
...
| Implementation commits | `b2b49a0c` (Drawer, keyboard, filter-chip fixes and regressions), `12784c46` (listbox focusability warning correction)                                                                                                                           |
| Owned paths            | This report and `tests/opus-accessibility-audit/**`; `Drawer` + keyboard model; `FilterChipBase` + all four dropdown consumers; `GalleryFilterSheet`; colocated regression tests and harnesses. No `BaseModal` or `ErrorModal` production files. |
...
- **Contract violated:** `docs/architecture/escape-routing.md`, ownership rule 1
  — _"Browser fullscreen and a focused input or popup keep the first Escape
  press."_
- **Cause (source):** `Drawer.svelte:485-497` installs a window-level
  ...
  _drawer_. It never consults `shouldDeferEscapeShortcut(document)` — the exact
...
- **Evidence (measured):** `escape-ownership.audit.test.ts` →
  _"leaves a focused text input inside a Drawer owning the first Escape press"_ —
...
- **Contract violated:** `escape-routing.md` rules 2 and 3 — _"The most recently
  opened registered modal or drawer claims the key"_ and _"one Escape press
  never closes two layers."_ It does worse than closing two: it closes the wrong
  one.
...
- **Evidence (measured):** `escape-ownership.audit.test.ts`
  - _"control: a BaseModal on its own closes on Escape from a focused field"_ —
    **passes**. BaseModal's own path is sound.
  - _"stacked modal is the layer dismissed — focus on a button in the modal"_ and
    _"— focus in the modal's text field"_ — both fail with
...
- **Bounded fix:** make `Drawer.handleKeydown` defer unless this drawer is the
  top layer of the _shared_ registry it already registers with
...
- **Why it still matters:** a keyboard or switch user pressing an arrow inside a
  ...
  and the sheet _should_ grow that handler. Today it neither navigates nor is
...
!shortcut.preserveDrawers && hasOpenDrawers()) dismissTopDrawer();` runs
  ...
  - Create registers bare `ArrowUp/Down/Left/Right` as single-key shortcuts with
    ...
    still stubs (`debug.log(... "not yet implemented")`), so the _only_ visible
...
- **Census (corrected in review pass).** The first version of this report used a
  ...
  `SupportModal`'s (which _does_ pass `labelledBy`) were misread as unnamed. The
...
target keeps the intentional "single key pressed _outside_ an open drawer
...
The seven specs that pass in _both_ columns are the guard cases: single-key
...
1. **The fix was silenced, not made correct.** The original push carried a
   ...
   _different_ rule than the one that actually fired.
...
reported. Contrast against the authenticated app's _animated_ background art was
...
- Route-level end-to-end verification. The pass-2 seven-tier measurement covers
  the touched primitive geometry only; contrast and pass-1 measurements were
