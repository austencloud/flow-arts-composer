...
The audit suite is green by construction: every assertion that pins _current_
behaviour passes, and every assertion describing a _contract the codebase
already states_ is marked `it.fails`. See "Quarantine convention" below for why
...
from a generated corpus sequence via `extractLeftSoloProp` →
`soloPropToSequence`:
...
1. _Immediately, cheap:_ extend the normalizer's refusal set with a
...
2. _Properly:_ add presence to `StepPairingData` (e.g.
...
**Correction.** An earlier revision of this report claimed plane was the _only_
...
**`birthday` blast radius, stated honestly.** For an _existing_ library document
...
known consumers _do_ coerce: `browse-date.ts:12` and
...
(or a decision that there is no bound). _Then_ a range check next to the
...
actionable defect. Everything else malformed _is_ refused with a typed error —
...
- **No corpus access, and no fixture that substitutes for it.** Every claim
  about live data is a claim about _shape_, never about counts. The
...
- **Firestore write paths were read, not exercised.** P1 and P2 trace through
  `library-repository.saveSequence`; the tests reproduce the _pure_ operation
