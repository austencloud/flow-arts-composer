...
export function getPreviewCacheKey(
  ...
  const gv = `${opts.showTnD ? "V1" : "V0"}${opts.showElemental ? "E1" : "E0"}${opts.showPlacements ? "P1" : "P0"}${opts.showGrid === false ? "G0" : "G1"}${opts.showHandColorKey === false ? "K0" : "K1"}${opts.showPropTnD ? "T1" : "T0"}`;
