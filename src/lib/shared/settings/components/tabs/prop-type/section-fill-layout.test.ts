...
describe("sectionFillLayout", () => {
  for (const [viewport, box] of Object.entries(BOXES)) {
    it(`fits the catalogue inside the picker at ${viewport}`, () => {
      const layout = sectionFillLayout({ counts: COUNTS, labels: LABELS, ...box });
  ...
  it("keeps the tiered grid when the host is too short for readable tiles", () => {
    expect(
      sectionFillLayout({ counts: COUNTS, labels: LABELS, width: 775, height: 380 })
    ).toBeNull();
  });

  it("waits for a measured box", () => {
    expect(
      sectionFillLayout({ counts: COUNTS, labels: LABELS, width: 0, height: 890 })
    ).toBeNull();
    expect(
