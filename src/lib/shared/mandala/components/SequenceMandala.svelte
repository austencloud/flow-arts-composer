<script lang="ts">
  ...
  function driftEase(t: number): number {
    const base = (1 - Math.cos(t * Math.PI)) / 2;
    const wander =
      Math.sin(t * Math.PI * 5) * Math.pow(t * (1 - t), 1.5) * 0.18;
    return base + wander;
  }
  ...
  function tidalEase(t: number): number {
    ...
    const secondary =
      Math.sin(t * Math.PI * 2) * 0.1 * (1 - Math.abs(2 * t - 1));
  ...
  const effectiveTipEnds = $derived(
    tipEnds ?? pairTipEnds(leftPropType, rightPropType)
  );
  ...
  const pathOptions = $derived(
    getMandalaPathOptions(pathShape, effectiveTipEnds)
  );
  ...
  const calculatedPaths = $derived.by((): MandalaPaths | null => {
    ...
    // otherwise the same mandala is visibly stretched before its PNG appears.
    const overrides = exportRaster
      ? tipDx === undefined
        ? undefined
        : { dx: effectiveDx, dy: 0 }
      : tipDx === undefined
        ? animatedTipOverrides
        : { dx: effectiveDx, dy: 0 };
  ...
  const useCanvas = $derived(
    exportRaster || animate || activeMorph !== null || changeMorphActive
  );
  ...
  function draw(): void {
    ...
    const logicalSize = Math.max(
      1,
      Math.round(canvas.getBoundingClientRect().width) || size
    );
    const device = Math.min(
      MAX_BACKING,
      Math.max(1, Math.round(logicalSize * dpr * SUPERSAMPLE))
    );
    ...
    if (typeof OffscreenCanvas !== "undefined") {
      if (
        !scratch ||
        scratch.a.width !== device ||
        scratch.a.height !== device
      ) {
        scratch = {
        ...
        : (() => {
            ...
            return {
              glow: {
                ...
                bloomBlur:
                  DEFAULT_OVERLAP_CONFIG.bloomBlur * renderScale * ratio,
              },
            };
          })()),
      offsetX: 0,
