<script lang="ts">
  ...
  let {
    ...
    onApply?: (placement: PictographData) => void | Promise<void>;
    onApplyPlacement?: (
  ...
  const builtPlacement = $derived.by((): StartPlacementPlacement | null => {
    ...
    return {
      ...
      leftPropType,
      rightPropType,
      id: "start-built-placement",
    };
  });

  const builtPictograph = $derived.by(() => {
    if (!builtPlacement) return null;
    // Center placements do not have a canonical TKA placement name. Assemble
    ...
    return startPlacementManager.createStartPlacementFromLocations(
      builtPlacement
    );
  });
  ...
  const placementLabel = $derived(
    builtPictograph ? getStartPlacementDisplayLabel(builtPictograph) : ""
  );
...
<div class="placement-builder" data-testid="build-start-placement">
  <div class="builder-layout">
    ...
    <div class="builder-controls">
      ...
      <p class="sr-only" aria-live="polite" aria-atomic="true">
        {placementLabel ? `Placement recognized: ${placementLabel}.` : ""}
      </p>
    </div>
  </div>
</div>
...
<style>
  .placement-builder {
    ...
    container: placement-builder / size;
    display: grid;
  ...
  @container placement-builder (min-width: 700px) {
    .builder-layout {
      flex-direction: row;
      width: min(100%, 56rem);
  ...
  @container placement-builder (max-height: 520px) {
    .builder-layout {
      padding: 12px;
      gap: 12px;
    }
  ...
  @container placement-builder (min-width: 600px) and (max-height: 400px) {
    .builder-layout {
      flex-direction: row;
  ...
  @container placement-builder (min-width: 1000px) and (min-height: 560px) {
    .builder-layout {
