<script lang="ts">
  ...
  const hi = (letter: string): Cell => ({ data: codexSlotData(letter, "high"), letter, slot: "high" });
  const lo = (letter: string): Cell => ({ data: codexSlotData(letter, "low"), letter, slot: "low" });
...
<div class="q-page">
  ...
  <div class="turn small" style="left:{100 * S}px; top:{560 * S}px; width:{120 * S}px">0 / 1</div>
  ...
  {#each rowDefs as row, ri (ri)}
    {#each row.cells as cell, ci (ci)}
      ...
      <div class="cl" style="left:{colLeft(row.x0, ci) * S}px; top:{(row.y + SIZE + 5) * S}px; width:{SIZE * S}px">
        <span class="tka">{cell.letter}</span>{#if cell.slot === "high"}<sup>1</sup>{:else}<sub>1</sub>{/if}{#if cell.oc}<span class="oc">{cell.oc}</span>{/if}
      </div>
    {/each}
  {/each}
</div>
