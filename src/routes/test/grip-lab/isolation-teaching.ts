...
export const POSE_CHANNELS = [
  "turn", "lean", "pitch", "pelvisX", "pelvisY", "pelvisZ",
  "elbowX", "elbowY", "elbowZ", "tipX", "tipY", "tipZ",
] as const;
export type PoseChannel = typeof POSE_CHANNELS[number];
export type TeachingPose = Record<PoseChannel, number>;
export interface TeachingKey extends TeachingPose { phase: number }
...
const NEUTRAL: TeachingPose = {
  turn: 0, lean: 0, pitch: 0, pelvisX: 0, pelvisY: 0, pelvisZ: 0,
  elbowX: -0.2, elbowY: -0.8, elbowZ: 0.35,
...
export function defaultTeachingKeys(): TeachingKey[] {
  return [
    ...
    { ...NEUTRAL, phase: 1 },
    { ...NEUTRAL, phase: 2 },
    { ...NEUTRAL, phase: 3, turn: 1.5, lean: 0.10, pelvisX: -0.035, pelvisZ: 0.025,
      elbowY: -0.6, elbowZ: 1, tipX: -0.08, tipZ: -0.05 },
    { ...NEUTRAL, phase: 3.5, turn: 1.3, lean: 0.05, pelvisX: -0.0175, pelvisZ: 0.0125,
      tipX: -0.10, tipY: 0.07, tipZ: -0.025 },
  ];
}
export function sampleTeachingPose(phase: number, keys: readonly TeachingKey[]): TeachingPose {
  return Object.fromEntries(POSE_CHANNELS.map((channel) =>
    [channel, sampleIsolationChannel(phase, keys, (key) => key[channel])],
  )) as TeachingPose;
}
...
export function upsertTeachingKey(keys: readonly TeachingKey[], phase: number, changes: Partial<TeachingPose>): TeachingKey[] {
  const t = wrapStaffIsolationPhase(Math.round(wrapStaffIsolationPhase(phase) * 1000) / 1000);
  const pose = sampleTeachingPose(t, keys);
  for (const channel of POSE_CHANNELS) {
    const next = changes[channel];
    if (next !== undefined && Number.isFinite(next)) {
      pose[channel] = Math.max(-channelLimit(channel), Math.min(channelLimit(channel), next));
    }
    pose[channel] = Math.round(pose[channel] * 10000) / 10000;
  }
  return [...keys.filter((key) => Math.abs(key.phase - t) >= 0.005), { ...pose, phase: t }]
...
export function authoredBodyPose(pose: TeachingPose): AuthoredContactPose {
  return {
    ...
    torsoYawRad: pose.turn, torsoLeanRad: pose.lean, torsoPitchRad: pose.pitch,
...
export function allowedTipOffset(pose: TeachingPose, toleranceM: number): [number, number, number] {
...
export function encodeTeachingKeys(keys: readonly TeachingKey[]): string {
  return JSON.stringify(keys.map((key) => [key.phase, ...POSE_CHANNELS.map((channel) =>
    Math.round(key[channel] * 10000) / 10000)]));
}
export function decodeTeachingKeys(raw: string | null): TeachingKey[] {
  if (!raw || raw.length > 24000) return defaultTeachingKeys();
  try {
    const rows: unknown = JSON.parse(raw);
    if (!Array.isArray(rows) || !rows.length || rows.length > 100) return defaultTeachingKeys();
    let keys: TeachingKey[] = [];
    for (const row of rows) {
      if (!Array.isArray(row) || row.length !== POSE_CHANNELS.length + 1 ||
          !row.every((value) => typeof value === "number" && Number.isFinite(value)) ||
          row[0] < 0 || row[0] >= 4) return defaultTeachingKeys();
      keys = upsertTeachingKey(keys, row[0], Object.fromEntries(POSE_CHANNELS.map((channel, i) => [channel, row[i + 1]])));
    }
    return keys;
  } catch { return defaultTeachingKeys(); }
}
